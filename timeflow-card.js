 ...
 * Generated on 2025-07-24T15:27:42.383Z
 * 
...
class ConfigValidator {
  ...
  static validateConfig(config) {
    ...
    booleanFields.forEach(field => {
      if (config[field] !== undefined && !this.isValidBooleanInput(config[field])) {
        errors.push(`Invalid ${field}. Must be a boolean (true/false), entity ID, or template.`);
      }
    });
    ...
    textFields.forEach(field => {
      if (config[field] && !this.isValidTextInput(config[field])) {
        errors.push(`Invalid ${field}. Contains potentially unsafe characters.`);
      }
    });
  ...
  static isValidTextInput(value) {
    if (this.isTemplate(value)) return true;
    if (typeof value !== 'string') return true; // Allow non-strings
    
    // Disallow common script injection patterns
    const forbiddenChars = /<|>|javascript:/i;
  ...
  static isValidStylesInput(styles) {
    ...
    const validKeys = ['card', 'title', 'subtitle', 'progress_circle'];
    for (const key in styles) {
      if (!validKeys.includes(key)) return false;
      if (!Array.isArray(styles[key])) return false;
      
      // Validate each style rule
      for (const rule of styles[key]) {
        if (typeof rule !== 'string' && typeof rule !== 'object') return false;
        if (typeof rule === 'object' && rule !== null) {
          for (const prop in rule) {
            if (typeof prop !== 'string' || typeof rule[prop] !== 'string') return false;
          }
        }
      }
    }
...
class DateParser {
  ...
  static parseISODate(dateString) {
    ...
    // 1. Native Date.parse() - Fast and usually reliable for ISO 8601
    let timestamp = Date.parse(dateString);
    if (!isNaN(timestamp)) {
      // Validate the parsed date to avoid incorrect interpretations
      const d = new Date(timestamp);
      if (this.isValidDate(d, dateString)) {
        return timestamp;
      }
    }

    // 2. Regex-based manual parsing - More robust for variations
    try {
      const parts = dateString.match(
        /(\d{4})-(\d{2})-(\d{2})[T\s](\d{2}):(\d{2}):(\d{2})(?:\.(\d{3}))?([Zz]|([+-])(\d{2}):(\d{2}))?/
      );
      ...
      if (parts) {
        ...
        const millisecond = parseInt(parts[7], 10) || 0;

        // Create date in UTC to handle timezones correctly
        timestamp = Date.UTC(year, month, day, hour, minute, second, millisecond);

        if (parts[8] && parts[8].toUpperCase() !== 'Z') {
          const tzSign = parts[9] === '-' ? -1 : 1;
          const tzHour = parseInt(parts[10], 10);
          const tzMinute = parseInt(parts[11], 10);
          const tzOffset = (tzHour * 60 + tzMinute) * 60 * 1000;
          timestamp -= tzSign * tzOffset;
        }
    ...
    // 3. Fallback to new Date() - Last resort, can be inconsistent
   ...
   */
  static isValidDate(dateObj, originalString) {
    // Check if the year from the parsed date matches the original string
    // This helps catch cases where browsers misinterpret the date
    const year = dateObj.getUTCFullYear();
    return originalString.startsWith(year.toString());
  }
}
