...
const BUNDLE = path.join(__dirname, '..', 'timeflow-card.js');
const CARD_TAG = 'timeflow-card';
...
async function testTimerLookedUpOncePerPass() {
  const mount = await mountCard(
    { type: 'custom:timeflow-card', title: 'Timer', auto_discover_alexa: true },
...
async function testWatchedEntityChangeWakesTheCard() {
  const mount = await mountCard(
    { type: 'custom:timeflow-card', title: 'Timer', timer_entity: 'sensor.echo_dot_next_timer' },
...
async function testListyStyleCarriesTimerEntityOver() {
  const context = loadBundle(createClock(Date.UTC(2026, 7, 31, 12, 0, 0)));
  context.CustomEvent = class { constructor(type, init = {}) { this.type = type; this.detail = init.detail; } };
  const Editor = context.customElements.get('timeflow-card-editor');
  const editor = new Editor();
  editor._config = { type: 'custom:timeflow-card', timer_entity: 'timer.pasta', auto_discover_alexa: true };
...
async function testBrandTextColourReachesTheRow() {
  const mount = await mountCard({
    type: 'custom:timeflow-card', style: 'listy', title: 'Timers',
